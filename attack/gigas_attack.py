...
class Feature(object):
    def __init__(self, seq_a):
        ...
        self.changes = []
import json
import attack.projected_gradient_descent as pgd
...
class Adv_attack:
    def __init__(self, vqa_model,pretrain_model,tokenizer,device,correct_idx_list,correct_pred_list,USE_model,itm_model,exp_file):
        ...
        ### generated question
        self.VLM_question_root = self.exp_file + "/VQG_question——dict.json"
        with open(self.VLM_question_root, 'r') as json_file:
            self.questions_dict = json.load(json_file)
    ...
    def bert_attack(self,batch,tgt_pos,score,gth):
        ...
        text_bank=[]
        sim_list=[]
        for _,top_index in enumerate(list_of_index):
    ...
    def VLM_sent(self,q_id,image,gth):
        success = 0
        text_cand = self.questions_dict.get(str(int(q_id)))
        for j in range (len(text_cand)):
            temp_text = text_cand[j]
            answer_ids, topk_ids, topk_probs, = self.black_model(image, temp_text,
                                                            self.answer_candidates, train=False,
                                                            inference='rank',
                                                            k_test=128)
            result = []
            for ques_id, answer_id in zip(q_id, answer_ids):
                result.append({"question_id": int(ques_id.item()), "answer": self.answer_list[answer_id]})
            ans_after_attack=result[0]['answer']
            if ans_after_attack != gth:
                success=1
                return text_cand[:j+1],success
        return text_cand,success
    
    def get_iter(self,adv_text,adv_img):
        similarity_list = []
        adv_text = list(set(adv_text))
        for text in adv_text:
            similarity = self.white_model.get_similarity(adv_img,text)
            similarity_list.append(similarity)
        if similarity_list == []:
            adv_text = [self.batch['question'][0]]
            similarity = self.white_model.get_similarity(adv_img,adv_text[0])
            similarity_list.append(similarity)
        similarity_list = normalize_similarity_scores(similarity_list)
        if len(similarity_list) != len(adv_text):
            raise ValueError("Difference Length in similarity_list and adv_text")
        _, adv_text_sorted = filter_and_sort_similarity(similarity_list,adv_text)
        text_bank = adv_text_sorted
        if len(adv_text) > self.single_stg_step:
            adv_text = adv_text[:self.single_stg_step]
        iters = int((self.total_stg_step - self.single_stg_step))
        iters_base = int((self.total_stg_step - self.single_stg_step) / len(adv_text))
        result_dict = {sentence: iters_base for sentence in adv_text}
        remaining_iters = iters - len(adv_text)
        if remaining_iters > 0 and len(text_bank) != 0:
            text_bank_index = 0
            while remaining_iters > 0:
                current_text = text_bank[text_bank_index % len(text_bank)]
                if current_text in result_dict.keys():
                    result_dict[current_text] += 1
                    remaining_iters -= 1
                    text_bank_index += 1
        if remaining_iters > 0:
            text_bank_index = 0
            while remaining_iters > 0:
                current_text = adv_text[text_bank_index % len(adv_text)]
                if current_text in result_dict.keys():
                    result_dict[current_text] += 1
                    remaining_iters -= 1
                    text_bank_index += 1
        iterations = [result_dict[sentence] for sentence in adv_text]
        return adv_text,iterations
    ...
    def evaluate(
        ...
        for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            ...
            que_id = str(batch['question_id'][0].cpu().numpy())
            ...
            # Uni-modal Image Attack
            torch.set_grad_enabled(True)
            adv_img = pgd.projected_gradient_descent(self.pgd_attack, adv_img, 0.125, 0.01, self.single_stg_step,
                                                         np.inf,clip_min=-1.0,clip_max=1.0,
                                                         y=[ori_txt_feats, ori_img_feats, None, None, None],
                                                         time=0, ori_x=ori_img,method='GIGAS')
            ...
            if out_v_ans!=self.blip_ans_table[str(int(batch['question_id'][0]))]:
                ...
                if len(self.acc_list) % 100 == 0 and len(self.acc_list) != 0:
                    print(f'ASR of {str(len(self.acc_list))} samples:', sum(self.acc_list) / len(self.acc_list), flush=True)
                continue
            # Uni-modal Text Attack
            ...
            generate_sent,generate_success = self.VLM_sent(batch['question_id'],ori_img,gth)
            if success==1:
                self.acc_list.append(1)
            elif generate_success == 1:
                self.acc_list.append(1)
            else:
            # Multi-modal Attack
                adv_text = adv_text + generate_sent
                text_bank,iters_list = self.get_iter(adv_text,adv_img)
                idx = 0
                ...
                while True:
                    ...
                    if out_v_ans != self.blip_ans_table[str(int(batch['question_id'][0]))]:
                        ...
                        break
                    torch.set_grad_enabled(True)
                    adv_img = pgd.projected_gradient_descent(self.pgd_attack, adv_img, 0.125, 0.01, iters_list[idx -1],
                                                         np.inf, clip_min=-1.0,clip_max=1.0,
                                                         y=[ori_txt_feats, ori_img_feats, None, None, None],
                                                         time=1, ori_x=ori_img,method='GIGAS')
                    torch.set_grad_enabled(False)
...
def filter_and_sort_similarity(similarity_list, adv_text):
    better_sentence = []
    for sim, text in zip(similarity_list, adv_text):
        if sim <= 0.75 and sim >= 0.25:
            better_sentence.append((sim, text))
    better_sentence_sorted = sorted(better_sentence, key=lambda x: x[0], reverse=True)
    sorted_sim, sorted_adv_text = zip(*better_sentence_sorted) if better_sentence_sorted else ([], [])
    return list(sorted_sim), list(sorted_adv_text)